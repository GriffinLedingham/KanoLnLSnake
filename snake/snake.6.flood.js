...
function getMove(state) {
  ...
  // Instantiate flood class
  let flood = new FloodFill()
  ...
  if(head.x > 0) {
    ...
    if(leftTile == EMPTY_TILE || leftTile == FOOD) {
      ...
      dirScores.left = -Infinity
    }
    if(leftTile == FOOD) {
      dirScores.left += 100
    }
    dirScores.left += flood.flood(board,head.x-1,head.y,99)
  }
  // Check right
  if(head.x < width - 1) {
    ...
    if(rightTile == EMPTY_TILE || rightTile == FOOD) {
      ...
      dirScores.right = -Infinity
    }
    if(rightTile == FOOD) {
      dirScores.right += 100
    }
    dirScores.right += flood.flood(board,head.x+1,head.y,99)
  }
  // Check up
  if(head.y > 0) {
    ...
    if(upTile == EMPTY_TILE || upTile == FOOD) {
      ...
      dirScores.up = -Infinity
    }
    if(upTile == FOOD) {
      dirScores.up += 100
    }
    dirScores.up += flood.flood(board,head.x,head.y-1,99)
  }
  // Check down
  if(head.y < height - 1) {
    ...
    if(downTile == EMPTY_TILE || downTile == FOOD) {
      ...
      dirScores.down = -Infinity
    }
    if(downTile == FOOD) {
      dirScores.down += 100
    }
    dirScores.down += flood.flood(board,head.x,head.y+1,99)
  }
